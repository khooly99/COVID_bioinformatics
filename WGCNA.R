...
n5 <- read.table("n5.txt", header=TRUE, row.names = NULL)
...
n5_meta <- as.data.frame(t(read.csv("n5_meta.csv", header=TRUE, row.names = 1)))
...
colnames(n5_meta) <- c("Sample_source_name", "Sample_organism", "Sample_characteristics_ct", "Sample_characteristics_ch", "Sample_title")
n5_meta <- n5_meta[match(colnames(n5[,-1]), rownames(n5_meta)),]
n5_meta$Sample_characteristics_ch[which(n5_meta$Sample_characteristics_ch == "disease state: SARS-CoV-2 positive")] <- "positive"
n5_meta$Sample_characteristics_ch[which(n5_meta$Sample_characteristics_ch == "disease state: Control")] <- "negative"
...
sampleID <- as.data.frame(c(row.names(n1_meta), row.names(n2_meta), row.names(n3_meta),
                            row.names(n4_meta), row.names(n5_meta)))
...
condition <- as.data.frame(c(n1_meta$Sample_characteristics_ch, n2_meta$Sample_characteristics_ch,
                             ...
                             n5_meta$Sample_characteristics_ch))
colnames(condition) <- "Condition"
batch <- c(rep(1, ncol(n1[,-1])), rep(2, ncol(n2[,-1])), rep(3, ncol(n3[,-1])), rep(4, ncol(n4[,-1])), rep(5, ncol(n5[,-1])))
...
# Build network
softPower <-  19 # Chosen in the graphs before
temp_cor <- cor
cor <- WGCNA::cor
...
# Choosing modules MEblack=0.37
module_key <- module_df
module_key <- module_key %>% filter(module == "MEblack")
rownames(module_key)
...
colnames(tMEs) <- c("MEblack", "condition")
tMEs$condition <- as.factor(tMEs$condition)
ggplot(tMEs, aes(x=condition, y=MEblack, color=condition)) +
  geom_boxplot()
...
MMsig <- data.frame(ModuleMembership[module_sig, module_key$gene]>0.8, ModuleMembership[module_sig, module_key$gene])
colnames(MMsig) <- c("MMsig", "MM")
MMsig <- MMsig %>%
  dplyr::filter(MMsig == "TRUE")

# Identify genes with high Gene Significance (GS > 0.2)
GSsig <- data.frame(GeneSignificance[module_key$gene, 1]>0.2, GeneSignificance[module_key$gene, 1])
colnames(GSsig) <- c("GSsig", "GS")
GSsig <- GSsig %>%
  dplyr::filter(GSsig == "TRUE")

# Combine MM and GS criteria to identify and extract hub genes
sigHubGenes <- data.frame(ModuleMembership[module_sig, module_key$gene]>0.8, GeneSignificance[module_key$gene, 1]>0.2,
