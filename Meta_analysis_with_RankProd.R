...
LFC <- LFC1 %>%
  ...
  full_join(LFC5, by= "GeneName") 
colnames(LFC) <- c("GeneName", "Study1", "Study2", "Study3",
                  "Study4", "Study5")
row.names(LFC) <- LFC[,1]
LFC <- LFC[,-1]
...
colnames(cor.df) <- str_replace(colnames(cor.df), "Study5", "GSE184610")
...
colnames(ranks.up.mx2) <- str_replace(colnames(ranks.up.mx2), "Study5", "GSE184610")
...
colnames(ranks.down.mx2) <- str_replace(colnames(ranks.down.mx2), "Study5", "GSE184610")
...
pheatmap(mx_down_top20[1:6,],
         cluster_rows = FALSE,
         cluster_cols = FALSE,
         show_rownames = TRUE,
         show_colnames = TRUE,
         fontsize_row = 10,
         main = "Top 6 Down-regulated Genes",
         color = heat_colors)
